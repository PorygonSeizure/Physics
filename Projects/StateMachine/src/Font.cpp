...
Font::Font(const char* trueTypeFontFile, unsigned short fontHeight)
	: m_glyphData(nullptr),
	m_glHandle(0), m_pixelBufferHandle(0),
	m_sizeOfBytesX(0), m_sizeOfBytesY(0)
{
	FILE* file = nullptr;
	fopen_s(&file, trueTypeFontFile, "rb");
	if (file != nullptr)
	{
		unsigned char* TTFBuffer = new unsigned char[4096 * 1024];

		fread(TTFBuffer, 1, 4096 * 1024, file);
		fclose(file);
		...
		unsigned char* tempBitmapData = (GLubyte*)glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, m_sizeOfBytesX * m_sizeOfBytesY, GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);

		m_glyphData = new stbtt_bakedchar[256];
		memset(m_glyphData, 0, sizeof(stbtt_bakedchar) * 256);
		stbtt_BakeFontBitmap(TTFBuffer, 0, fontHeight, tempBitmapData, m_sizeOfBytesX, m_sizeOfBytesY, 0, 256, (stbtt_bakedchar*)m_glyphData);
