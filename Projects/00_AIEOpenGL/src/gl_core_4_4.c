...
{
	static const struct machHeader* image = NULL;
	NSSymbol symbol;
	char* symbolName;
	if (NULL == image)
...
{
	HMODULE glMod = NULL;
	PROC func = wglGetProcAddress((LPCSTR)name);
	if (TestPointer(func))
		return func;
...
static ogl_StrToExtMap extensionMap[1] = { {"", NULL, NULL} };

static int extensionMapSize = 0;

static ogl_StrToExtMap* FindExtEntry(const char* extensionName)
{
	int loop;
	ogl_StrToExtMap* currLoc = extensionMap;
	for (loop = 0; loop < extensionMapSize; ++loop, ++currLoc)
	{
		if (!strcmp(extensionName, currLoc->extensionName))
			return currLoc;
	}
...
static void ProcExtsFromExtList()
{
	...
	_ptrc_glGetIntegerv(GL_NUM_EXTENSIONS, &numExtensions);
...
int OGLGetMinorVersion()
{
	if (!majorVersion)	//Yes, check the major version to get the minor one.
		GetGLVersion();
	return minorVersion;
}

int OGLIsVersionGEQ(int majorVersion, int minorVersion)
{
	if (!majorVersion)
		GetGLVersion();
